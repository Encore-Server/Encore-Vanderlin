	...
	if(!mode)
		...
		switch(vote_type)
			...
			if("endround")
				var/rng = rand(1, 1000)
				if(rng > 200) // 80%
					initiator_key = pick("Visires", "Akan", "Gani", "Mjallidhorn", "Valdala", "Hertannea", "Alaqkh'shir", "Ird", "Goler Kanh", "Pomette")
				else if(rng > 50) // 15%
					initiator_key = pick("the Deceiver Gods", "the Archdevils", "the Seven Sages")
				else
					initiator_key = "the One Envy"
