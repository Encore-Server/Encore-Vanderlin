...
MOBS_SUBSYSTEM_DEF(deceivers_mobs)
	name = "Necropolis Mobs"
	priority = FIRE_PRIORITY_TOMB_MOBS
	flags = SS_KEEP_TIMING | SS_NO_INIT

/datum/controller/subsystem/mobs/deceivers_mobs/build_currentrun()
	...
	for(var/z_index in GLOB.tomb_z_levels)
		...
		for(var/mob/living/client_mob as anything in clients_here)
			...
			if(!turf)
				continue
			for(var/datum/spatial_grid_cell/cell as anything in SSspatial_grid.get_cells_in_range(turf, DECEIVERS_PROCESSING_TILE_RANGE))
				if(seen_cells[cell])
					continue
...
/datum/controller/subsystem/mobs/deceivers_mobs/proc/get_random_mob()
	var/list/mobs = list()
	for(var/z_index in GLOB.tomb_z_levels)
		mobs |= mobs_by_zlevel[z_index]
	return pick(mobs)

#undef DECEIVERS_PROCESSING_TILE_RANGE
