/datum/objective/personal/steal_items
	name = "Steal Items"
	category = "Deceiver Gods' Chosen"
	triumph_count = 2
	rewards = list("2 Triumphs", "Deceiver Gods grows stronger", "Pickpocketing knowledge", "Deceiver Gods blesses you (+1 Speed)")
	var/stolen_count = 0
...
/datum/objective/personal/steal_items/proc/on_item_stolen(datum/source, mob/living/victim)
	...
	if(stolen_count >= required_count)
		...
		to_chat(owner.current, span_notice("Item stolen! Steal [required_count - stolen_count] more to complete Deceiver Gods' objective."))

/datum/objective/personal/steal_items/complete_objective()
	. = ..()
	to_chat(owner.current, span_greentext("You have stolen enough items to complete Deceiver Gods' objective!"))
...
/datum/objective/personal/steal_items/update_explanation_text()
	explanation_text = "Steal [required_count] item\s from others to prove your cunning to Deceiver Gods!"
