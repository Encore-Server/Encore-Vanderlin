...
/datum/round_event_control/graggar_culling/canSpawnEvent(players_amt, gamemode, fake_check)
	...
	if(!.)
		return FALSE
	if(GLOB.patron_follower_counts["Archdevils"] < 2)
		return FALSE
...
/datum/round_event/graggar_culling/start()
	...
	for(var/i in 1 to max_pairs)
		...
		bordered_message(first_chosen, list(
			...
			span_red("Weak should feed the strong, that is Archdevils's will. Prove that you are not weak by eating the heart of [span_notice(second_chosen.real_name)], the [second_chosen.job] and gain unimaginable power in turn. Fail, and you will be the one eaten."),
		))
		if(grand_culling)
		...
		bordered_message(second_chosen, list(
			span_userdanger("YOU ARE GRAGGAR'S CONTESTANT!"),
			span_red("Weak should feed the strong, that is Archdevils's will. Prove that you are not weak by eating the heart of [span_notice(first_chosen.real_name)], the [first_chosen.job] and gain unimaginable power in turn. Fail, and you will be the one eaten."),
		))
		if(grand_culling)
