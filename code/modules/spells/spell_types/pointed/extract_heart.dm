/datum/action/cooldown/spell/extract_heart
	...
	desc = "An unholy rite to claim hearts as a tribute to Archdevils. Only works on fresh corpses."
...
/datum/action/cooldown/spell/extract_heart/cast(mob/living/carbon/human/cast_on)
	...
	if(cast_on.stat != DEAD)
		to_chat(owner, span_warning("The weakling still pulses with life! Archdevils demands you finish them off first!"))
		return
	...
	owner.visible_message(span_warning("[owner] rips [cast_on]'s heart out with a roar!"), \
						span_red("You present the heart to Archdevils! The God chuckles upon this offering."))
